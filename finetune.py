...
class CNN():
    ...
    def test_close(self, new_model, X_test, y_test):
        ...
        print(self.count)
        print('Testing with best model...')
        score, acc, top5Acc = new_model.evaluate(X_test, y_test, batch_size=64)
    
        from utility import class_ranks, ranks_KNN
        y=list()
        for i in y_test:
            y.append(np.argmax(i))
        ranks = class_ranks(new_model, X_test, y, 10, preds=None)
...
if __name__ == "__main__":
    ...
    opts = testOpts(trainData='/work/swanlab/fafrin2/hackrf/our_day1/our_day1/',
                    tuneData='/work/swanlab/fafrin2/hackrf/our_day2/our_day2/',
                    location='symbols',
                    testType='tsn',
                    modelType='complex',
                    modelPath='/work/swanlab/fafrin2/usrp/hackrf/after_fft/crelu/s288/res_out/modelDir/IQ_model_our_day1_complex_symbols_slices_1000_startIdx_100000_stride_288_len_288_STFT_64.h5_0.71')
    # if opts.useGpu:
    #     os.environ['CUDA_VISIBLE_DEVICES'] = "0"
    dataOpts = load_slice_IQ.loadDataOpts(opts.tuneData, opts.location, num_slice=1000, slice_len=288,
                                          start_idx=0, stride=288, mul_trans=True, window=64, dataType='IQ')
